...
def _require_mask_schema(stage: str, path: Path, hdr: Any) -> None:
    """Require the compact <=8-char MASK schema header cards.

    We store schema cards in the *primary* header (and also in the MASK HDU).
    Enforcing this prevents silent drift in bit meanings.
    """

    # Key names are generated by maskbits.header_cards(prefix="SCORP").
    mkv = hdr.get("SCORPMKV", None)
    if mkv is None:
        raise ProductContractError(
            stage=stage,
            path=str(path),
    ...
    # Require declared names for bits we currently define (0..9).
    # Do not hard-validate the exact string to remain robust to minor casing.
    for i in range(0, 10):
        k = f"SCORPMB{i}"
        if k not in hdr:
            raise ProductContractError(
...
def validate_mef_product(path: str | Path, *, stage: str) -> None:
    ...
    _require_mask_schema(stage, p, hdr)
