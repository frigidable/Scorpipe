...
This module is the *single source of truth* for execution semantics across:
- CLI (scorpio-pipe run --execute)
- GUI (via scorpio_pipe.ui.pipeline_runner shim)
- workflow/doit (workflow/dodo.py adapter)

Invariants enforced here:
- Deterministic resume/force (stage_state + stage hash)
- QC gate as the only downstream blocker
- Boundary contract validation for products
- done.json + stage_state are always written (ok/skip/blocked/fail/cancelled)
"""
...
@dataclass(frozen=True)
class StageSpec:
    """Lightweight stage/task specification (P0-A5).

    This is intentionally *non-invasive*: stage implementations do not depend on it.
    The engine may use it for deterministic validation/provenance and UI/help text.

    ``validate_globs`` serves two roles:

    1) **Execution contract**: declares which on-disk artifacts count as this
       task's outputs (used by tests/docs).
    2) **Boundary validation**: for strict product kinds (MEF/spec1d/lambda_map)
       the engine validates every matched file after a successful task.
    """

    key: str
    title: str
    contract_kind: str = 'none'  # 'mef'|'spec1d'|'lambda_map'|'none'
    validate_globs: tuple[str, ...] = ()
    # Optional list of machine-readable product keys (see scorpio_pipe.products)
    # that are considered outputs of this task.
    output_keys: tuple[str, ...] = ()
    hash_inputs_policy: str = 'default'


# Canonical task list (single source for CLI/UI/doit naming).
...
# Minimal per-task spec table. This is meant to evolve gradually.
STAGE_SPECS: dict[str, StageSpec] = {
    # NOTE: validate_globs are declared for *all* canonical tasks (P0-A7), even
    # if the task is not (yet) covered by strict boundary validation.
    'manifest': StageSpec(
        key='manifest',
        title='Manifest / provenance',
        contract_kind='none',
        validate_globs=('manifest/manifest.json',),
        output_keys=('manifest', 'products_manifest', 'timings'),
    ),
    'superbias': StageSpec(
        key='superbias',
        title='Superbias / MasterBias',
        contract_kind='none',
        validate_globs=(
            'superbias.fits',
            'master_bias*.fits',
            'master_bias_index.json',
        ),
        output_keys=(
            'superbias_fits',
            'master_bias_fits',
    ...
    'superflat': StageSpec(
        ...
        title='Superflat',
        contract_kind='none',
        validate_globs=('superflat.fits',),
        output_keys=('superflat_fits',),
    ),
    'flatfield': StageSpec(
        key='flatfield',
        title='Flat-Fielding',
        contract_kind='mef',
        # Boundary validation: only MEF FITS.
        validate_globs=('*_flat.fits',),
        output_keys=('flatfield_done',),
    ),
    'cosmics': StageSpec(
        key='cosmics',
        title='Cosmics Cleaning',
        contract_kind='none',
        validate_globs=(
            'summary.json',
            'obj/summary.json',
            'obj/clean/*_clean.fits',
            'obj/clean/*_clean_mask.fits',
            'sky/summary.json',
            'sky/clean/*_clean.fits',
            'sky/clean/*_clean_mask.fits',
        ),
        output_keys=('cosmics_summary',),
    ),
    'superneon': StageSpec(
        key='superneon',
        title='Superneon',
        ...
        validate_globs=(
            'superneon.fits',
            'superneon.png',
            'peaks_candidates.csv',
            'superneon_shifts.csv',
            'superneon_shifts.json',
            'superneon_qc.json',
        ),
        output_keys=(
            'superneon_fits',
    ...
    'lineid_prepare': StageSpec(
        key='lineid_prepare',
        title='LineID Preparation',
        contract_kind='none',
        validate_globs=(
            'manual_pairs_template.csv',
            'manual_pairs_auto.csv',
            'lineid_report.txt',
        ),
        output_keys=('lineid_template_csv', 'lineid_auto_csv', 'lineid_report_txt'),
    ),
    'wavesolution': StageSpec(
        key='wavesolution',
        title='Wavelength Solution',
        contract_kind='lambda_map',
        # Boundary validation: only lambda_map FITS.
        validate_globs=('lambda_map.fits',),
        output_keys=(
            'lambda_map',
    ...
    # Boundary contract product stages
    'sky': StageSpec(
        key='sky',
        title='Sky Subtraction',
        contract_kind='mef',
        validate_globs=('*_skysub_raw.fits', '*_skymodel_raw.fits', 'sky_preview*.fits'),
        output_keys=('sky_done',),
    ),
    'linearize': StageSpec(
        key='linearize',
        title='Linearization',
        contract_kind='mef',
        validate_globs=('*_skysub.fits', '*_skymodel.fits', 'lin_preview*.fits'),
        output_keys=('linearize_done', 'lin_preview_fits', 'linearize_qc'),
    ),
    'stack2d': StageSpec(
        key='stack2d',
        title='2D Stacking',
        contract_kind='mef',
        validate_globs=('stack2d*.fits', 'stacked2d*.fits'),
        output_keys=('stacked2d_fits',),
    ),
    'extract1d': StageSpec(
        key='extract1d',
        title='1D Extraction',
        contract_kind='spec1d',
        validate_globs=('spec1d*.fits',),
        output_keys=('spec1d_fits',),
    ),
    'qc_report': StageSpec(
        key='qc_report',
        title='QC Report',
        ...
        validate_globs=(
            'qc/qc_report.json',
            'qc/qc_report.html',
            'manifest/qc_report.json',
            'manifest/qc_report.html',
            'index.html',
            'manifest/products_manifest.json',
        ),
        output_keys=('qc_json', 'qc_html', 'products_manifest'),
    ),
    'navigator': StageSpec(
        key='navigator',
        title='Navigator',
        contract_kind='none',
        validate_globs=('ui/navigator/index.html', 'ui/navigator/data.json'),
    ),
}
...
# Public helpers (used by CLI/UI/doit adapters)

def done_dir_for_task(run_root: Path, task: str) -> Path:
    """Return the task directory that holds *done.json* for this task.

    The returned directory is always based on the *canonical* task name.
    """

    return _done_dir_for_task(Path(run_root), canonical_task_name(task))


def done_json_path_for_task(run_root: Path, task: str) -> Path:
    """Return the done.json path for a task (canonicalized)."""

    return done_dir_for_task(run_root, task) / "done.json"
...
def _validate_task_products(task: str, out_dir: Path) -> None:
    """Validate boundary contract for *this* task outputs (hard-fail).

    Migration policy (P0-B5): we start with high-value products and then expand
    coverage gradually.

    The engine uses :data:`STAGE_SPECS` to decide what to validate.
    ...
    task
        Canonical task key.
    out_dir
        Directory of the stage outputs (``run_root/NN_slug``).
    """

    # NOTE: ``out_dir`` is already the stage directory; do NOT call stage_dir() here.

    spec = STAGE_SPECS.get(task)
    if spec is None:
        return
    ...
    globs = tuple(getattr(spec, 'validate_globs', ()) or ())

    cands: list[Path] = []
    try:
        for g in globs:
            gg = str(g)
            # rglob supports recursive patterns without needing '**/'.
            for p in out_dir.rglob(gg):
                if p.is_file():
                    cands.append(p)
    except Exception:
        cands = []
    ...
    if not cands:
        if kind == 'lambda_map':
            try:
                cands = [p for p in sorted(out_dir.rglob('lambda_map.fits')) if p.is_file()]
            except Exception:
                cands = []
        elif kind == 'mef':
            p0 = out_dir / f"{task}.fits"
            if p0.is_file():
                cands = [p0]
    ...
    if kind == 'lambda_map':
        for p in uniq:
            validate_lambda_map_product(p, stage=task)
        return
...
def plan_sequence(
    ...
    *,
    ...
    # P0-B3: reference context is part of provenance and affects stage hashing.
    # This is a hard contract: missing/unhashable references must fail.
    try:
        from scorpio_pipe.refs.context import ensure_reference_context

        resources_dir = cfg.get("resources_dir") if isinstance(cfg, dict) else None
        _ref_ctx = ensure_reference_context(cfg, resources_dir=resources_dir)
        ref_context_id = str(_ref_ctx.get("context_id") or "")
    except Exception:
        raise
    ...
    for raw in task_names:
        ...
        expected_hash = compute_stage_hash(
            ...
            reference_context_id=ref_context_id,
        )
...
def run_sequence(
    ...
    *,
    ...
    # Accept config dicts, YAML paths, and lightweight wrappers that expose
    # `cfg` and/or `cfg_path` attributes (e.g. GUI RunContext).
    if config_path is None:
        for attr in ('cfg_path', 'config_path'):
            if hasattr(cfg_or_path, attr):
                v = getattr(cfg_or_path, attr)
                try:
    ...
    # P0-B3: reference context is part of provenance and affects stage hashes.
    # The context must exist before planning/running any stage.
    ref_ctx: dict[str, Any] | None = None
    ref_context_id: str = ""
    ...
    # P0-B3: create/update manifest/reference_context.json (hard contract).
    try:
        from scorpio_pipe.refs.context import ensure_reference_context

        resources_dir = cfg.get("resources_dir") if isinstance(cfg, dict) else None
        ref_ctx = ensure_reference_context(cfg, resources_dir=resources_dir)
        ref_context_id = str(ref_ctx.get("context_id") or "")
    except Exception as e:
        raise RuntimeError(f"Reference context build failed: {e}") from e
        ...
        *,
        ...
        try:
            ...
            additions: dict[str, Any] = {
                ...
                "effective_config": _stage_cfg_for_hash(cfg, task) if isinstance(cfg, dict) else {},
                "outputs_list": outputs_list,
                "provenance_bundle": prov_paths if isinstance(prov_paths, dict) else {},
                "reference_context": {
                    "context_id": ref_context_id,
                    "path": "manifest/reference_context.json",
                },
            }
            if products_sanity:
    ...
    def _expected_hash(task: str) -> str:
        return compute_stage_hash(
            stage=task,
            stage_cfg=_stage_cfg_for_hash(cfg, task),
            input_paths=_input_paths_for_hash(cfg, task, out_dir),
            reference_context_id=ref_context_id,
        )
    ...
    for it in plan:
        if cancel_token is not None and cancel_token.cancelled:
            ...
            _upsert_stage_done_json(it.task, status="cancelled", stage_hash=_expected_hash(it.task), reason="Cancelled", started_utc=datetime.now(timezone.utc).isoformat(), finished_utc=datetime.now(timezone.utc).isoformat(), duration_s=0.0)
            record_stage_result(
                out_dir,
                it.task,
                status="cancelled",
        ...
        if it.action == "skip":
            ...
            try:
                ...
                update_after_stage(cfg, stage=t, status="skipped", stage_hash=_expected_hash(t))
            except Exception:
                pass
            _upsert_stage_done_json(t, status="skipped", stage_hash=_expected_hash(t), reason=str(it.reason or "skipped"), started_utc=datetime.now(timezone.utc).isoformat(), finished_utc=datetime.now(timezone.utc).isoformat(), duration_s=0.0)
            continue
        ...
        try:
            ...
            _upsert_stage_done_json(t, status="blocked", stage_hash=_expected_hash(t), reason=ge.summary(), started_utc=datetime.now(timezone.utc).isoformat(), finished_utc=datetime.now(timezone.utc).isoformat(), duration_s=0.0, err_code="QC_GATE", err_msg=ge.summary())
            record_stage_result(
                out_dir,
                t,
                status="blocked",
        ...
        try:
            ...
            _upsert_stage_done_json(
                ...
                err_code=("CONTRACT" if isinstance(e, ProductContractError) else type(e).__name__),
                err_msg=(f"{e.code}: {e.message}" if isinstance(e, ProductContractError) else str(e)),
                trace=tb,
            )
            record_stage_result(
                out_dir,
