...
def _load_cfg_any(cfg: Any) -> Dict:
    ...
    if hasattr(cfg, "__dict__"):
        d = dict(cfg.__dict__)
        # если frames — не dict, попробуем привести
...
def build_superbias(cfg: Any, out_path: str | Path | None = None) -> Path:
    """
    Строит superbias как среднее по bias-кадрам (для bias это нормально и устойчиво).
    ...
    combine = str(calib_cfg.get("bias_combine", "mean")).lower()
    ...
    # (bias sets are usually small enough that this is safe and removes
