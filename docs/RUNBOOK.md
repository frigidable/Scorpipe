# RUNBOOK — Scorpio Pipe
Этот файл — короткая шпаргалка «как прогнать пайплайн» и где искать продукты.

## Workspace layout (контракт)

Рабочая папка одного прогона всегда имеет вид:

```
workspace/<night>/<object>_<disperser>_<run_id>/
```

Где:
- `<night>` — дата ночи в формате `DD_MM_YYYY` (например `30_12_2025`).
- `<run_id>` — двухзначный номер (`01`, `02`, ...).
- В корне run-папки **обязателен** `run.json` — «паспорт» запуска (источник истины).

### Санитайзер имён (Windows-safe)

Чтобы workspace был переносимым между ОС, имена `object` и `disperser` автоматически
приводятся к безопасному виду:

- запрещённые в Windows символы `< > : " / \ | ? *` заменяются на `_`;
- пробелы и последовательности `_` схлопываются;
- ведущие/замыкающие точки/пробелы удаляются.

Если `run.json` расходится с именем папки, GUI покажет предупреждение и предложит кнопку
**Fix run.json**.
