\# Scorpio Pipe / Scorpipe# Scorpio Pipe / Scorpipe
...
\- научное ядро v5.x: \*\*sky\_sub → linearize → stack2d → extract1d\*\*,
...
6\. `lineid\_prepare` → \*\*LineID GUI\*\* (привязка) → сохранить пары
...
scorpio-pipe inspect --data-dir <PATH\_TO\_NIGHT>



\# валидация конфига

scorpio-pipe validate --config <WORK\_DIR>/config.yaml



\# диагностика окружения/ресурсов (опционально с автофиксом)

scorpio-pipe doctor --config <WORK\_DIR>/config.yaml --fix



\# список ожидаемых продуктов для данного config

scorpio-pipe products --config <WORK\_DIR>/config.yaml



\# автоконфиг + (опционально) запуск неинтерактивных стадий

scorpio-pipe run --data-dir <PATH\_TO\_NIGHT> --object "<OBJECT\_NAME>" --execute
...
Подробности: \[`INSTALL.md`](INSTALL.md), \[`GET\_SETUP\_EXE.md`](docs/GET\_SETUP\_EXE.md).
...
\- \*\*lineid\_prepare\*\* — подготовка к LineID (таблицы, preview, путь wavesol).
...
\- \*\*qc\_report\*\* — отчёт (HTML + JSON) и сводные метрики.
...
\- `frames` описывает выбранные файлы (science/calibs) и setup (`frames.\_\_setup\_\_`).
...
\- `work/qc/products\_manifest.json` — индекс продуктов (в т.ч. per‑exposure деревья)

\- `work/qc/index.html`, `work/qc/qc\_report.json` — отчёт
...
\- `work/lineid/hand\_pairs.txt` — сохранённые ручные пары

\- `work/08\_wavesol/` (или `work/08\_wavesol/<disperser>/`) — продукты решения (JSON/PNG/λ‑карта)



\### Science

\- `work/10\_linearize/lin\_preview.fits` (+ PNG)

\- `work/09\_sky/preview.fits` (+ PNG)

\- `work/11\_stack/stacked2d.fits` (+ coverage.png)

\- `work/12\_extract/spec1d.fits` (+ PNG)
...
\- \*\*“setup.bat мигнул и исчез”\*\* → используйте `setup\_debug.bat` (пишет `setup\_debug.log`).  

&nbsp; См. \[`SETUP\_TROUBLESHOOTING.md`](docs/SETUP\_TROUBLESHOOTING.md).
...
\- `setup.bat` / `setup\_debug.bat` — сборка/запуск на Windows

\- `packaging/windows/scorpipe.spec` — PyInstaller

\- `packaging/windows/scorpipe.iss` — Inno Setup (установщик)

\- `.github/workflows/windows\_release.yml` — сборка релиз‑артефактов (CI)
